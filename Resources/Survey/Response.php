	...
	private function processSurveyData($survey_data){
		...
		foreach ($survey_data as $question_id => $question_data) {
			...
			if(isset($question_data['subquestions'])){
				foreach ($question_data['subquestions'] as $sub_question_id => $sub_question_data) {
					//Account for slight differences in subquestion formatting
					if(!isset($sub_question_data['id'])){
						$data_to_process = array($sub_question_id => array('options' => $sub_question_data));
					}
					else{
						$data_to_process = array($sub_question_id => $sub_question_data);
					}
					//process subquestions as if they are normal questions
					$process_sub_data = $this->processSurveyData($data_to_process);

					//certain complex questions (like some tables) will have sub_question that should be grouped on the parent id
					if($sub_question_id > 10000){
						$payload_data[$question_id][$sub_question_id] = array_pop($process_sub_data);
					}
					else{
						$payload_data[$sub_question_id] = array_pop($process_sub_data);
					}
				}
			}
			...
			elseif (isset($question_data['options'])) {
				foreach ($question_data['options'] as $option_id => $option_data) {
					$payload_data[$question_id][$option_id] = isset($option_data['answer']) ? $option_data['answer'] : null;
				}
			}
			else{
				$payload_data[$question_id] = isset($question_data['answer']) ? $question_data['answer'] : null;
			}
		}
		return $payload_data;
	}
